...
自动从 arXiv 抓取 AI/机器人领域论文，调用 OpenAI 兼容 API 做基础分析（标签/评级/中文摘要）和按需深度阅读（Q&A），存入 SQLite，通过 Flask Web 界面浏览并生成报告。Python 3.11 / Flask / SQLite / APScheduler / arxiv-py / OpenAI SDK / PyMuPDF。
...
无 lint/format/CI 配置；唯一测试文件是 `tests/test_ai_provider_config.py`（41 个用例，覆盖配置/认证/抓取/分析路由/模板安全）。
...
**AI 任务路由。** 有三个独立 AI 任务：`basic_analysis`、`deep_reading`、`report_summary`，各自路由到自己的供应商/模型/参数。调用前用 `get_ai_task_config(task_key)` 取配置、`get_prompt_profile(task_key)` 取 Prompt 前缀，**参数必须用 `build_chat_completion_kwargs()` 构建**——绝不在业务代码里硬编码 `temperature`/`max_tokens`（思考模型会自动省略采样参数并改用 reasoning/thinking 字段）。
...
**Prompt 缓存设计。** 稳定的 Prompt 前缀放在 `prompt_profiles`，论文标题/作者/摘要/PDF 全文作为**最后一条独立 JSON message** 传入。不要把动态论文内容拼回稳定 instruction，否则会破坏 prompt cache 命中率。
