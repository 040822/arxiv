...
自动从 arXiv 抓取具身智能与人工智能领域最新论文，调用 AI 快速阅读分析（标签、翻译、摘要、简评），生成 Web 报告，并提供 Flask 浏览界面。论文评级由用户手动维护，CLI 仍可按需生成 Markdown 报告。
...
评级不再由 AI 自动生成。新分析论文默认 `0星`，可在论文详情页手动设置 0~5 星；历史 AI 评级会备份到 `legacy_ai_rating` 后清零，不参与当前展示和排序。
...
**分析流程重构**
- 批量分析（每日定时/手动批量）改为轻量模式：不下载 PDF，仅基于摘要生成标签/翻译/简评；评级由用户手动维护
