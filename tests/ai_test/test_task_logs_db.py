    ...
    def test_task_logs_include_ordered_pipeline_steps(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                initialize_task_log_steps(log_id, [
                    ("fetch", "抓取论文"),
                    ("analyze", "基础分析"),
                ])
                set_task_log_step_status(log_id, "fetch", "running", "正在抓取")
                set_task_log_step_status(log_id, "fetch", "success", "抓取 2 篇")
                finish_task_log(log_id, "warning", "完成但有警告")

                logs, total = get_task_logs(task_name="daily_pipeline")
    ...
    def test_startup_reconciliation_interrupts_orphaned_task_runs(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                running_id = start_task_log("daily_pipeline", "每日定时任务启动")
                initialize_task_log_steps(running_id, [
                    ("fetch", "抓取论文"),
                    ("analyze", "基础分析"),
                ])
                set_task_log_step_status(running_id, "fetch", "running", "正在抓取")
                finished_id = start_task_log("fetch", "手动抓取")
                finish_task_log(finished_id, "success", "已完成")

                interrupted = interrupt_running_task_logs("服务重启，任务已中断")
                logs, _ = get_task_logs()
