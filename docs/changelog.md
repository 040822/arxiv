...
### 报告邮件
- 新增每日报告邮件发送，可在设置页配置 SMTP、收件人、主题模板和站点地址，并支持手动测试发送最近日报
- 邮件正文改为收件箱友好的摘要版：先展示 `report_summary` AI 导读，再突出推荐分 `>80` 的重点精读论文，其余论文最多展示 20 篇快速速览
- SMTP 发送复用现有网络代理配置，代理启用时通过 HTTP CONNECT 隧道连接邮件服务器
- 邮件发送失败只记录 `email_report` 任务日志和最近错误，不中断每日抓取、分析、报告生成和 WebDAV 备份
