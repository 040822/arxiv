...
- 数据库 schema v4 新增 `users`、`audit_events`，阅读清单、聊天、练习、导入和 LLM 用量增加用户归属
- 旧 settings 管理密码安全迁入 admin 账号；用户表成为凭据唯一真源，旧字段在数据库提交后备份并移除
- Flask session 改为 `user_id + session_version`，30 天滑动有效；改密、重置、停用或删除立即撤销旧会话
- 全站 cookie 写请求增加 session-backed CSRF；登录按 IP 与规范化用户名限流且使用统一失败提示
- 成员可维护个人学习数据、修改共享评分/标签、生成共享深度阅读并公开导入论文；管理操作保持 admin 专属
- 私有查询增加所有权/IDOR 防护；管理员仅查看成员私有记录数量汇总
- 新增用户管理、审计日志、论文私有数据删除保护及 WebDAV 隐私告警
- `v0.6.0` 标注在提交 `89ad882`，作为模块化重构完成基线

## v0.6.0 — 模块化重构基线（2026-08-12）

- 完成 4.1/4.2 模块化重构与 settings 守卫，稳定基线提交 `89ad882`
