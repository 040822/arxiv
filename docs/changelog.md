...
### 模型供应商与功能路由解耦
- 模型供应商只管理 API Key、Base URL 与模型列表缓存；模型、输出长度、采样参数和思考模式统一迁入各功能模型路由
- 功能路由模型字段支持下拉建议与手动输入，并可按供应商刷新模型列表
- 每个功能路由新增独立连接测试，明确返回测试的功能、供应商、模型、耗时和思考能力提示；测试使用当前未保存草稿
- 设置页补充 Presence/Frequency Penalty 解释和七类功能的非思考模型 Temperature/Top P 推荐值
...
### vision 愿愿页内容扩充
- 新增「部署与落地成本」4 格说明（本地私有部署 / 数据可控 / 模型自选 / 低维护），回应部署与维护顾虑
...
### 报告邮件防重复发送
- 每日任务发送前检查 `last_sent_report_date`，同一日报已成功发送时跳过 AI 导读和 SMTP，避免重复投递
- 邮件失败不更新去重日期，后续每日任务仍可重试
- 手动测试发送保持可重复执行，但不计入每日任务去重
