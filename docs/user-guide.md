...
- 按日期列出所有报告
- 点击查看详情：统计概览、分类分布、热门标签、高分论文、全部论文列表
...
- **基础分析**（批量）：在任务管理页点击「🤖 开始分析」，只用摘要生成标签/评级/翻译
- **完整分析**（单篇）：在论文详情页点击「📝 生成报告」，下载 PDF 生成 Q&A 深度阅读
...
A: 数据库文件在 `data/papers.db`，备份只需复制该文件：
```bash
