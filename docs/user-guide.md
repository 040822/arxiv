...
- 新成员或被重置密码的成员首次登录后只能修改密码或退出；新密码至少 8 个字符。
...
首页保持公开浏览。未登录时首页右上角显示「登录」；登录后会显示显示名和明确的 `@用户名`，并提供「修改密码」和「退出登录」。登录页和改密页的密码输入框都可以使用眼睛按钮显示或隐藏密码。普通成员可从首页账号入口自行修改密码，首次登录的临时密码也必须在这里更换。

登录失败限流按来源 IP 和规范化用户名分别计算：任一维度在滚动 15 分钟内累计 5 次失败后，第 6 次请求返回 HTTP 429，并通过 `Retry-After` 和 JSON `retry_after` 告知等待秒数；登录页会显示倒计时。成功登录会清理当前 IP 和用户名的失败记录。计数只保存在单个 Flask 进程内存中，进程重启会清空，多 worker 之间不共享；系统默认不信任客户端伪造的 `X-Forwarded-For`。
...
登录后从首页右上角的账号入口进入「修改密码」，填写当前密码、新密码和确认密码。新密码至少 8 个字符；保存成功后旧会话版本立即失效并自动保留当前登录状态。输入错误或两次新密码不一致时，页面会直接提示原因。
...
3. 选择抓取方式：
   ...
   - **按日期**：选择「指定日期」精确抓取某天
   - **默认**：日期和天数都留空时抓取最近 1 天
4. 点击「📥 开始抓取」

> 指定日期优先于抓取天数。系统会重叠查询日期窗口并依靠数据库去重，既避免重复入库，也能在下一次运行时补抓此前失败的窗口。
