...
- **数据库**：统计信息、存储路径、WebDAV 云同步备份、报告邮件发送
- **管理**：密码保护
...
如在「设置 → 数据库 → 报告邮件发送」启用自动发送，每日任务会在报告生成后通过 SMTP 发送 HTML 邮件。邮件发送失败只会记录错误，不会中断抓取、分析、日报生成和云备份。

## 报告邮件发送

进入「设置 → 数据库」，填写 SMTP 服务器、端口、加密方式、用户名、密码或授权码、发件人、收件人、主题模板和站点地址。点击「测试发送」会使用最近一份已生成日报告验证配置；打开「启用每日自动发送报告邮件」后，每日定时任务会自动发送。
