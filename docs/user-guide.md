...
在「功能模型路由」中，可以为基础分析、深度阅读、论文对话、论文问答练习等功能分别选择供应商、模型、思考强度和输出长度。论文对话与问答练习默认更适合长上下文/高智能模型，因为它们会把论文全文作为稳定上下文放入请求。
...
邮件内容是专门为收件箱设计的精简版：开头展示 AI 导读，推荐分高于 80 的论文进入「重点精读」，其他论文最多展示 20 篇「快速速览」。收件人支持逗号、分号或换行分隔。`site_url` 用于把邮件里的站内论文和完整报告链接转成可点击的完整地址。
