...
| 功能 | Temperature | Top P |
|------|-------------|-------|
| PDF 元数据提取 | 0.1 | 1.0，不发送 |
| 基础分析 | 0.2 | 1.0，不发送 |
| 深度阅读 | 0.2 | 1.0，不发送 |
| 报告导读 | 0.3 | 1.0，不发送 |
| 个性化推荐 | 0.2 | 1.0，不发送 |
...
通常只调整 Temperature 或 Top P 中的一个。Presence Penalty 根据某个 token 是否已经出现施加一次惩罚，正值鼓励引入新内容；Frequency Penalty 根据 token 已出现的次数累积惩罚，正值更强地减少重复措辞。两者默认都关闭，结构化提取和评分任务通常保持为 0。
